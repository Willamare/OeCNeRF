...
class Blender(Dataset):
    ...
    def _load_renderings(self, config):
        ...
        for idx, frame in enumerate(tqdm(meta['frames'], desc='Loading Blender dataset', disable=self.global_rank != 0, leave=False)):
            ...
            # image.append(imageio.imread(fprefix))
            ...
            if self._use_tiffs:
                ...
                image = get_img('') / 255.
            # image = (np.array(image) / 255.).astype(np.float32)  # keep all 4 channels (RGBA)
            images.append(image)
        ...
        self.images = self.images[..., :3]  # Assuming images only have RGB channels
